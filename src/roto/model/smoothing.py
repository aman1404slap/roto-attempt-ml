"""Temporal smoothing of a predicted control-point track, before keys are chosen.

The model's per-frame error is approximately independent frame to frame, while the artist's
true track is piecewise smooth between sparse keys. Smoothing therefore removes a large part
of the noise while leaving genuine motion nearly untouched, which is what lets the keyframe
search see the structure it is meant to find.

**Which filter matters, and v1's choice was measurably the wrong one.** A boxcar (moving
average) attenuates exactly what an artist keys: a direction reversal, a contact frame, the
apex of a bounce. Those are motion extremes, and a moving average flattens extremes by
construction. Savitzky-Golay fits a low-order polynomial over the same window and takes its
value at the centre, so a locally quadratic peak survives -- it suppresses comparable noise
without shaving the peaks off.

At the ends of a track the polynomial is fitted to the first (or last) full window and
evaluated at the true offset, rather than the track being padded with a repeated edge value.
Edge replication flattens the first and last frames towards a constant, and the first frame
of a shape's life is one an artist almost always keys.
"""
