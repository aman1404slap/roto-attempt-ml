"""v1 training: one shared network over every element, batched a frame at a time.

Batches are drawn from a single element, because the shape count sets the query count and
mixing elements in one batch would mean padding a 4-shape element out to 1036. Each step
samples one element (weighted by frame count, so a 191-frame element is seen more often than
...
the crop's own pixel size makes one weight meaningful for every element. Element scales differ
by 35x in document terms -- ``px_per_norm`` runs from 124 to 4365 -- and a loss stated in those
units would spend all its capacity on the largest element and report that as progress.
...
from .data import ElementData, load_dataset
...
def train(dataset_root: str | Path, out_dir: str | Path,
    ...
    summary = {'n_params': n_params, 'steps': cfg.steps, 'elements': len(els),
