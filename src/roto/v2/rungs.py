"""The v2 ladder: one named configuration per charter S4 stage.

Each rung is a named constant rather than a set of flags assembled at a shell prompt, so a
result can be traced back to the exact configuration that produced it. The rungs live here
rather than in ``scripts/train_v2.py`` because two callers need the same catalogue and must
not drift: the script, and the Django service that launches a run on ECS. A rung defined in a
script is a rung the service can only re-type.

Importing this pulls in ``roto.v2.config`` and nothing else -- no torch. The service validates
an incoming run's rung against this catalogue before it launches anything, and that check must
not require the training stack.
...
DEFAULT_RUN = 's3a'
"""The live rung. The ladder below it is history, not a prerequisite.

S0 through S2C were each run to answer one question about one change, and each of those
questions is answered -- the results are in the design notes and the rungs are kept here so a
number can be traced to the configuration that produced it. A larger dataset does not reopen
them: more data cannot tell you what plan section 5's loss reweight cost geometry, because
that was measured against a fixed dataset on purpose.

So a run on a bigger archive starts at **S3A**, which is the one rung whose question is still
open. Re-running the ladder from S0 at 1,000 shots would cost days of GPU time to reproduce
conclusions that were never about data volume."""
