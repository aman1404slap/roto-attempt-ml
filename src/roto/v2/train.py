...
from .config import (AFFINE_SPACES, CROP, DOC, PAIRS, RANDOM, RUNS, SAMPLING_MODES,
                     TrainConfig)
...
def train(dataset_root: str | Path, out_dir: str | Path,
    ...
    # Which environment produced this and which dataset it saw. On the checkpoint as well as
    # the log because scoring loads the checkpoint, and the rule that a local run never enters
    # a results table has to be enforceable there. See roto.v2.provenance.
    prov = stamp(dataset_root)
    ckpt = {
    ...
    summary = {'provenance': prov,
               'n_params': n_params, 'steps': cfg.steps, 'layers': len(els),
