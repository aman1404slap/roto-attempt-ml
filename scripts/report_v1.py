...
The frame shown in each figure is chosen at random from that element's live range, with a
fixed seed. Picking the best frame would make the figures a highlight reel; picking the worst
would make them a bug report. A seeded random frame is the only choice that stays honest when
the numbers move.
"""
...
def main() -> None:
    ...
    for d in dirs:
        ...
        i = int(rng.integers(len(rec.frames)))
        frame = int(rec.frames[i])
